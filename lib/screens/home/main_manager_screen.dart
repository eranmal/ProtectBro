...
class _MainManagerScreenState extends State<MainManagerScreen> {
                              ...
                              final guards = guardSnap.data!.docs
                                  .map((d) => Guard(
                                      ...
                                      totalDifficultyScore:
                                          d['totalDifficultyScore'] ?? 0,
                                      lastShiftEnd:
                                          (d['lastShiftEnd'] as Timestamp)
                                              .toDate(),
                                      isActive: d['isActive'] ?? true,
                                      isCommander: d['isCommander'] ?? false))
                                  .toList();
                              _copyScheduleToClipboard(scheduleSnap.data!.docs,
                                  widget.groupName, guards);
                            },
                          );
                        });
                  }),
                      ...
                      builder: (context, scheduleSnap) {
                        ...
                        final guards = guardSnap.data!.docs
                            .map((d) => Guard(
                                ...
                                totalDifficultyScore:
                                    d['totalDifficultyScore'] ?? 0,
                                lastShiftEnd:
                                    (d['lastShiftEnd'] as Timestamp).toDate(),
                                isActive: d['isActive'] ?? true,
                                isCommander: d['isCommander'] ?? false))
                            .toList();
                        final stations = stationSnap.data!.docs.map((d) {
                          ...
                          return Station(
                              ...
                              guardsNeeded: data['guardsNeeded'] ?? 1,
                              isAllDay: data['isAllDay'] ?? true,
                              startHour: data['startHour'] ?? 0,
                              endHour: data['endHour'] ?? 24,
                              maxShiftMinutes: data['maxShiftMinutes']);
                        }).toList();
                        ...
                        return SingleChildScrollView(
                            ...
                            child: Column(children: [
                              ...
                              if (widget.isAdmin) ...[
                                ...
                                _buildSectionHeader(() => _addGuard(groupRef),
                                    onReset: () => _resetMetrics(groupRef),
                                    onBulkAdd: () =>
                                        _bulkAddGuardsDialog(groupRef)),
                                ...guards
                                ...
                                Padding(
                                    ...
                                    child: CyberButton(
                                        ...
                                        onPressed: () => _partialScheduleDialog(
                                            guards,
                                            stations,
      ...
      child: ListTile(
          ...
          trailing: Row(mainAxisSize: MainAxisSize.min, children: [
            IconButton(
                ...
                onPressed: () => _editGuardDialog(g, groupRef)),
            Switch(
                            ...
                            itemBuilder: (c, i) {
                              ...
                              return ListTile(
                                  ...
                                  leading: const Icon(Icons.chat_bubble_outline,
                                      size: 16, color: Color(0xFF00FF87)));
                            });
                      })),
              Padding(
                  ...
                  child: Row(children: [
                    ...
                    IconButton(
                        ...
                        onPressed: () {
                          ...
                          ref.collection('station_logs').add({
                            'station': sN,
                            'text': ctrl.text,
                            'sender': _selectedMyName ?? "אורח",
  ...
  Widget _buildSectionHeader(VoidCallback n,
      ...
      Padding(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
            ...
            if (onBulkAdd != null)
              IconButton(
                  icon: const Icon(Icons.group_add, color: Color(0xFF00B8FF)),
                  onPressed: onBulkAdd),
            IconButton(
  ...
  void _copyScheduleToClipboard(
      List<QueryDocumentSnapshot> docs, String groupName, List<Guard> guards) {
  ...
  void _partialScheduleDialog(
      List<Guard> gs, List<Station> ss, DocumentReference ref) {
        ...
        builder: (c) => StatefulBuilder(
            builder: (c, setS) => AlertDialog(
                    ...
                    content: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      DropdownButton<int>(
                          dropdownColor: const Color(0xFF111A11),
                    ...
                    actions: [
                      ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.pop(c);
                            _runPartialSchedule(gs, ss, ref, hour);
                          },
  ...
  void _runPartialSchedule(
      List<Guard> gs, List<Station> ss, DocumentReference ref, int hour) async {
    ...
    for (var doc in snap.docs) {
      if ((doc['start'] as Timestamp).toDate().isAtSameMomentAs(start) ||
          (doc['start'] as Timestamp).toDate().isAfter(start)) {
        final st = ss.firstWhere((s) => s.name == doc['station']);
        for (String n in doc['guards']) {
          sRefu[n] = (sRefu[n] ?? 0) + 1;
          dRefu[n] = (dRefu[n] ?? 0) + st.difficultyLevel;
        }
    ...
    for (var g in gs) {
      ...
      if (rem.isNotEmpty) {
        ...
        nLE = (rem.first['end'] as Timestamp).toDate();
      }
      await ref.collection('guards').doc(g.id).update({
        'totalShifts': FieldValue.increment(-(sRefu[g.name] ?? 0)),
        'totalDifficultyScore': FieldValue.increment(-(dRefu[g.name] ?? 0)),
        'lastShiftEnd': Timestamp.fromDate(nLE)
      });
    }
    var upGs = (await ref.collection('guards').get())
        .docs
        .map((d) => Guard(
            id: d.id,
            name: d['name'],
            ...
            lastShiftEnd: (d['lastShiftEnd'] as Timestamp).toDate(),
            isActive: d['isActive'] ?? true,
            isCommander: d['isCommander'] ?? false))
        .toList();
    ...
    for (var e in fM.entries) {
      await ref.collection('guards').doc(e.key).update({
        'totalShifts': e.value.totalShifts,
        'totalDifficultyScore': e.value.totalDifficultyScore,
        'lastShiftEnd': Timestamp.fromDate(e.value.lastShiftEnd)
      });
    }
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
  ...
  void _editShift(QueryDocumentSnapshot doc, List<Guard> allG,
      List<Station> allS, DocumentReference ref) {
                        ...
                        (i) => DropdownButtonFormField<String>(
                            ...
                            onChanged: (v) async {
                              ...
                              await ref
                                ...
                                'totalDifficultyScore':
                                    FieldValue.increment(-st.difficultyLevel)
                              });
                              await ref
                                  .collection('guards')
                                  .doc(newG.id)
                  ...
                  ElevatedButton(
                      ...
                      onPressed: () async {
                        List<String> names = ctrl.text
                            ...
                            .toList();
                        for (var n in names) {
                          await ref.collection('guards').add({
                            'name': n,
                            'totalShifts': s,
                            'totalDifficultyScore': d,
                            'lastShiftEnd': Timestamp.now(),
                    ...
                    actions: [
                      ElevatedButton(
                          style: ElevatedButton.styleFrom(
                              ...
                              foregroundColor: Colors.black),
                          onPressed: () {
                            ref.collection('guards').doc(g.id).update({
                              'totalShifts':
  ...
  void _showCodes(DocumentReference ref) async {
    var d = await ref.get();
    if (!mounted) return;
    showDialog(
        context: context,
    ...
    return Container(
        ...
        child: ListTile(
            ...
            trailing: IconButton(
                ...
                onPressed: () => _showLog(next['station'], ref))));
  }
  ...
  void _runSchedule(
      List<Guard> gs, List<Station> ss, DocumentReference ref) async {
    DateTime start = DateTime(
    ...
    for (var entry in updatedMap.entries) {
      await ref.collection('guards').doc(entry.key).update({
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("השיבוץ פורסם!", style: TextStyle(color: Colors.black)),
          backgroundColor: Color(0xFF00FF87)));
    }
  }
}
